...
const main = () => {
    ...
    const adjustLightValue = (e) => {
        light = e.target.classList.contains('left') ? light + 1 : light - 1;
        lightInput.value = light;
        colorChange()
        setLightThumb()
    }
    document.querySelector('.light-adjust .left').addEventListener('click', adjustLightValue);
    document.querySelector('.light-adjust .right').addEventListener('click', adjustLightValue);
    /*
